...
def usertxt (): 
    with open("user,txt", "a")as file:
        for x in customer_details [len(customer_details)-1]:
            file.write (str(x)+" ")
        print("User", customer_details, "has been registered successfully")
...
def append_name():
    ...
    customer_name=entry_customer.get().strip().capitalize()
    quantity=entry_quantity.get()
    customer_details.append([customer_name,receipt_number,item_Variable.get(),quantity])
    usertxt()
    # Clear input fields
